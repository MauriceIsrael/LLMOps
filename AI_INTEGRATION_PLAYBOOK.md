...
| `document-engine` | Compilateur pur HLD & assistance à la rédaction | **Pureté du compilateur (ADR-DE-05) & Garde sortante (ADR-DE-43 / ADR-SUITE-05 E2)** : `compile()` reste 100 % pur et offline. L'assistance prose (`POST /api/prose/suggest-batch`, ADR-DE-02) n'écrit jamais directement dans le `ProseStore`. Tout appel sortant vers le Hub est soumis au plafond obligatoire `KNOWLEDGE_HUB_MAX_OUTBOUND_CLASSIFICATION` (refus 403 `outbound-refused` par défaut). Priorité consignée au journal `DECISIONS.md` (clause E3). |

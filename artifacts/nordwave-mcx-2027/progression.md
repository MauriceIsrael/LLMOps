...
Gaps found: **29** · dispatched: **3** ·
held by the level gate: **26**.
...
| Held gap | Reason |
|---|---|
| G1_empty_section | subject floor-control is at L0_named, needs L2_decomposed |
| G1_empty_section | subject floor-control is at L0_named, needs L3_decided |
| G1_empty_section | subject media-distribution is at L0_named, needs L2_decomposed |
| G1_empty_section | subject lmr-interworking is at L0_named, needs L2_decomposed |
| G1_empty_section | subject subscriber-db is at L0_named, needs L2_decomposed |
...
```mermaid
graph TD
    GATE["Level gate: 26 gaps held"]:::held
...
The decomposition created **4 subjects** at `L0_named`,
advanced `mcx-services` to `L2_decomposed`, and released
**1** previously held gaps. Refinement is
...
## Act 5a — a declared conflict: Rui contests a specific statement

Rui was never asked this question. He contested statement `S-0005` four
...
|---|---|---|
| S-0005 | active | floor arbitration terminates in the MC service layer at the site |
| S-0006 | active | depends on a committed priority and pre-emption profile in t |
...
**6** subjects below `L3_decided`. The reason matters: the
...
|---|---|---|
| MCX Service Layer Decomposition (4 sub-domains) | decomposition | First occurrence of 3GPP MC service layer decomposition on m |

...
- **Phase 0 — Automated Anti-Fabrication Validator (§2 & Defect Fix 1 & 2)**: Created `scripts/eval/eval_config.json`, `scripts/eval/validate.py`, and `tests/eval/test_validator.py`. Verified strict `sys.exit(1)` on any error and `sys.exit(0)` on clean state. Updated `check_sources` to recalculate extracted text length and assert equality with `SOURCES.md`. Added unit test `test_validator_exit_code_non_zero_on_invalid_state` (7/7 tests passing).

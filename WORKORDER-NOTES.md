...
- **Phase 0 — Automated Anti-Fabrication Validator (§2 & Correction 1)**: Created `scripts/eval/eval_config.json`, `scripts/eval/validate.py`, and `tests/eval/test_validator.py`. Updated `check_sources` to recalculate extracted text length and assert equality with `SOURCES.md`. Added unit test `test_extracted_length_mismatch_rejection` (6/6 tests passing).
- **Phase 1 — Specification Acquisition & SHA-256 Hashing (§3 & Correction 2)**: Downloaded TS 22.179 Rel-13 (`22179-d30.zip` / `v13.3.0`), TS 23.179 Rel-13 (`23179-d50.zip`), TS 23.280 Rel-14+, TS 23.379 Rel-14+, and TS 24.380 Rel-14+. Extracted 2.8M characters of text. Documented version pairing timeline in `docs/eval/PROTOCOL.md`.
- **Phase 2 — Hierarchical Clause Parsing & Perimeter Check (§4 & Correction 3)**: Updated `scripts/eval/extract_clauses.py` to parse hierarchical clause numbers (`7.3.1`, `10.2.1`) and filter TOC lines. Counted **62 substantial floor control clauses in Stage 2** (threshold $\ge 10$: **PASSED**) and **101 in Stage 3**. Recorded counts in `docs/eval/CORPUS.md`.
